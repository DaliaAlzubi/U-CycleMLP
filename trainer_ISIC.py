...
def trainer_ISIC(args, model, snapshot_path,nodes_snapshot_path='ISIC_current_snapshot.pth'):
    seed=42
    ...
    writer = SummaryWriter(snapshot_path + '/ISIC_dataset_training_log')
    metrics_csv=[]
    best_model = None
    LAST_DICE_LOSS = float('-inf')
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        metrics_df.to_csv(os.path.join(snapshot_path, 'ISIC_metrics.csv'), index=False)
